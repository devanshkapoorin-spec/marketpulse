...
export default function AIScore({ ticker }: AIScoreProps) {
  ...
  const volumes = raw.map((d: any) => +d.volume)
  ...
  const result: CalcResult = calcAIScore({ rsi: lastRsi, macdHist: lastMacdHist, priceVsSMA20, priceVsSMA50, bbPctB: lastPctB, volumeRatio, change5d })

  const recColor =
    result.recommendation === 'Strong Buy' || result.recommendation === 'Buy' ? 'text-accent-green' :
    result.recommendation === 'Sell' || result.recommendation === 'Strong Sell' ? 'text-accent-red' :
    'text-yellow-400'
